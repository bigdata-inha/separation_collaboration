...
if args.id == 'cifar10':
    ...
    trainset = torchvision.datasets.CIFAR100(root='/mnt/disk2/workspace/Datasets/cifar100', train=True, download=True, transform=transform_train)
    trainloader = torch.utils.data.DataLoader(
        trainset, batch_size=128, shuffle=True, num_workers=2)

    testset = torchvision.datasets.CIFAR100(root='/mnt/disk2/workspace/Datasets/cifar100', train=False, download=True, transform=transform_test)
    testloader = torch.utils.data.DataLoader(
        testset, batch_size=100, shuffle=False, num_workers=2)
...
ood_valid_loader = torch.utils.data.DataLoader(ood_valid_dataset, batch_size=512, shuffle=False, num_workers=8)
...
ood_valid_loader2 = torch.utils.data.DataLoader(ood_valid_dataset2, batch_size=512, shuffle=False, num_workers=8)
...
if args.id == 'cifar10':
    net = torch.load("./check_model/fedavg_pretrained_model_iid_WideResNet_88_concat_flat.pth")
elif args.id == 'svhn':
    net = torch.load("./check_model/fedavg_pretrained_model_iid_WideResNet_96_concat_flat_svhn.pth")
elif args.id == 'cifar100':
...
def train(epoch):
    ...
    if args.rate == 0:
        for in_set in trainloader:
            ...
            loss = criterion(x, target.type(torch.LongTensor).to(device))
        ...
        for in_set, out_set in zip(trainloader, oe_loader):
            ...
            loss += 0.25* -(x[len(in_set[0]):].mean(1) - torch.logsumexp(x[len(in_set[0]):], dim=1)).mean()
...
for epoch in range(start_epoch, start_epoch+2):
    ...
    scheduler.step()
    if epoch == 1:
        torch.save(net, "./check_model/paper_final_result/mixup_collaboration_wideresnet.pth")